...
template <>
struct call_traits<std::string>
		: public internal::call_traits_ref<std::string> {};
template <>
struct call_traits<List> : public internal::call_traits_ref<List> {};
template <>
struct call_traits<Object> : public internal::call_traits_ref<Object> {};
...
public:
	...
	Value(const std::string &v)
			: type_(STRING_TYPE), string_(new std::string(v)) {}
	...
	// Merge object. Returns true if succeeded. Otherwise, |this| might be
...
class Lexer {
public:
	explicit Lexer(std::istream &is) : is_(is), lineNo_(1), columnNo_(0) {}
	...
	while (current(&c)) {
		...
		if (c == '\\') {
			...
			case '\n':
				if (braces == 0) {
					return Token(TokenType::ILLEGAL,
											 std::string("literal not terminated"));
				} else {
					while (current(&c) &&
								 (c == ' ' || c == '\t' || c == '\r' || c == '\n')) {
						next();
			...
			return Token(
					TokenType::ILLEGAL,
					std::string("found newline while parsing non-HIL string literal"));
		} else if (c == '"' && braces == 0) {
			if (hil)
...
inline Token Lexer::nextStringSingleQuote() {
	...
	while (current(&c)) {
		...
		if (c == '\'') {
			...
			return Token(TokenType::ILLEGAL,
									 std::string("found newline while parsing string literal"));
		}
...
inline bool hasSuffix(const std::string &s, const std::string &suffix) {
	...
	return std::mismatch(suffix.rbegin(), suffix.rend(), s.rbegin()).first ==
				 suffix.rend();
}
...
inline Token Lexer::nextHereDoc() {
	if (!consume('<'))
		return Token(TokenType::ILLEGAL,
								 std::string("heredoc didn't start with '<<'?"));
	if (!consume('<'))
		return Token(TokenType::ILLEGAL,
								 std::string("heredoc didn't start with '<<'?"));
	...
	if (!current(&c) || (current(&c) && c != '\n')) {
		return Token(TokenType::ILLEGAL,
								 std::string("invalid characters in heredoc anchor"));
	}
...
inline Value::Value(const Value &v) : type_(v.type_) {
	...
	case HIL_TYPE:
		...
		break;
	case LIST_TYPE:
		list_ = new List(*v.list_);
		break;
	case OBJECT_TYPE:
		object_ = new Object(*v.object_);
		break;
	default:
		assert(false);
		type_ = NULL_TYPE;
...
template <> struct Value::ValueConverter<Object> {
	...
	const Object &to(const Value &v) {
		v.assureType<Object>();
		return *v.object_;
	}
};

template <typename T> struct Value::ValueConverter<std::vector<T>> {
...
template <typename T> inline void Value::assureType() const {
	if (!is<T>())
...
inline bool Value::isHil() const { return getStringType() == StringType::Hil; }
...
inline void Value::write(std::ostream *os, const std::string &keyPrefix,
												 int indent) const {
	switch (type_) {
	case NULL_TYPE:
	...
	case OBJECT_TYPE:
		for (const auto &kv : *object_) {
			if (kv.second.is<Object>())
				continue;
			if (kv.second.is<List>() && kv.second.size() > 0 &&
					kv.second.find(0)->is<Object>())
				continue;
...
inline bool Value::mergeObjects(const std::vector<std::string> &keys,
																Value &added) {
	if (keys.size() == 0) {
		return false;
	}
...
inline Value *Value::ensureValue(const std::string &key) {
	...
	while (true) {
		internal::Token t = lexer.nextToken();
		if (key.size() > 0 && !(t.type() == internal::TokenType::IDENT ||
														t.type() == internal::TokenType::STRING)) {
...
inline void Parser::addError(const std::string &reason) {
	std::stringstream ss;
	ss << "Error:" << lexer_.lineNo() << ":" << lexer_.columnNo() << ": "
		 << reason << "\n";
	errorReason_ += ss.str();
}
...
inline Value Parser::parse() { return parseObjectList(false); }
...
inline bool Parser::parseKeys(std::vector<std::string> &keys) {
	...
	while (true) {
		...
		case TokenType::ILLEGAL:
			...
			return false;
		default:
			addError("expected IDENT | STRING | ASSIGN | LBRACE got: " +
							 token().strValue());
			return false;
		}
	}
	...
	while (true) {
		...
		if (needComma) {
			...
			case TokenType::RBRACK:
				break;
			default:
				addError("error parsing list, expected comma or list end, got: " +
								 token().strValue());
				return false;
			}
		}
...
static inline int isSpace(int c) { return (c == ' ' || c == '\t') ? 1 : 0; }
...
static inline std::string join(const std::vector<std::string> &elements,
...
inline bool Parser::unindentHeredoc(const std::string &heredoc,
																		std::string &out) {
	...
	for (const auto &line : lines) {
		...
		auto res = std::mismatch(whitespacePrefix.begin(), whitespacePrefix.end(),
														 line.begin());
